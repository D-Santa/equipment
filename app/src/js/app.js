...
var app = new Framework7({
  ...
  methods: {
    ...
    if (notifi.length>=1){
      ...
      {
        if (notifi[s].types=="2")types="<img src='static/fire2.gif' width='40' style='background-color:#ffffff'></img>"; 
        else
        types="";
      if (notifi[s].active==="0"){
      ht1+='<div data-id ="'+notifi[s].id+'" name="'+notifi[s].id_db+'" value="0" class="card"><div class="card-header" name="'+notifi[s].id_db+'" value="0" data-id ="'+notifi[s].id+'"><i>Заявка</i> № '+notifi[s].id+'<font color="red">Новые</font></div>'+
      '<div data-id ="'+notifi[s].id+'" value="0"name="'+notifi[s].id_db+'"  class="card-content card-content-padding"><div style="position:absolute;right:10px;margin-top:-18px">'+types+'</div><center><b>'+notifi[s].name+'</b></center><br><b>id: </b> '+notifi[s].id_db+'<br><b>Вид неисправности: </b> '+notifi[s].status+'<br><b>Расположение: </b> '+notifi[s].company+', '+notifi[s].place+'<br><b>Инициатор: </b> '+notifi[s].vrach+'<br></div>'+
